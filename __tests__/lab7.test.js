describe('Basic user flow for Website', () => {
  const siteURL = 'https://cse110-sp25.github.io/CSE110-Shop/';
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      const currProd = prodItemsData[i];
      if (currProd.title.length == 0) { allArePopulated = false; }
      if (!String(currProd.price).length) { allArePopulated = false; }
      if (currProd.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allInCart = await page.$$eval('product-item', (items) => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allNotInCart = await page.$$eval('product-item', (items) => {
